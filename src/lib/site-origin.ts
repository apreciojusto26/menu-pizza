...
function parseConfiguredOrigin(
  ...
  const candidate =
    addHttpsWhenMissing && !value.includes("://") ? `https://${value}` : value;
  ...
  if (url.pathname !== "/" || url.search || url.hash) {
    throw new SiteOriginError(
      `${source} must contain only an origin, without path, query, or fragment`,
    );
  }
  ...
  if (!isPublicDnsHostname(hostname) && !(allowLoopback && loopback)) {
    throw new SiteOriginError(
      `${source} must use a public DNS hostname in production`,
    );
  }
  if (
    url.protocol !== "https:" &&
    !(url.protocol === "http:" && allowLoopback && loopback)
  ) {
...
/**
 * Resuelve el único origen público de confianza usado en las URLs de
 * retorno/redirección que se le pasan a SumUp al crear un checkout. Un
 * SITE_URL explícito gana; el dominio de producción de Vercel es el
 * respaldo seguro de deploy. El host de la request NUNCA se confía fuera de
 * desarrollo local — de lo contrario, un atacante podría controlar a dónde
 * SumUp redirige tras el pago con solo mandar un header Host falso.
 */
export function resolveCanonicalOriginFromValues(
  values: CanonicalOriginValues,
): string {
  ...
  if (values.allowLocalRequestOrigin) {
    ...
    if (!isLoopbackHostname(hostname)) {
      throw new SiteOriginError(
        "Refusing non-loopback request origin without SITE_URL",
      );
    }
    if (requestUrl.username || requestUrl.password) {
      throw new SiteOriginError(
        "Local request URL must not contain credentials",
      );
    }
